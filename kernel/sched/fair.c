...
static int hmp_pre_up_migration_noti(int cpu);
static int hmp_pre_down_migration_noti(int cpu);
...
#ifdef CONFIG_SOFT_TASK_MIGRATION
		if (!(hmp_pre_up_migration_noti(new_cpu) & NOTIFY_STOP_MASK)) {
#endif//CONFIG_SOFT_TASK_MIGRATION
		hmp_next_up_delay(&p->se, new_cpu);
...
#ifdef CONFIG_SOFT_TASK_MIGRATION
			if (!(hmp_pre_down_migration_noti(new_cpu) & NOTIFY_STOP_MASK)) {
#endif//CONFIG_SOFT_TASK_MIGRATION
...
{
	return atomic_notifier_call_chain(&hmp_task_migration_notifier,
			HMP_PRE_UP_MIGRATION, (void *)(long)cpu);
}

static int hmp_pre_down_migration_noti(int cpu)
{
	return atomic_notifier_call_chain(&hmp_task_migration_notifier,
			HMP_PRE_DOWN_MIGRATION, (void *)(long)cpu);
}
#endif//CONFIG_SOFT_TASK_MIGRATION
/*
...
#ifdef CONFIG_SOFT_TASK_MIGRATION
				if (!(hmp_pre_up_migration_noti(target_cpu) & NOTIFY_STOP_MASK)) {
#endif//CONFIG_SOFT_TASK_MIGRATION
				get_task_struct(p);

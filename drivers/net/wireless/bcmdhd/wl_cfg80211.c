...
extern void init_roam(int ioctl_ver);
extern void reset_roam_cache(void);
extern void add_roam_cache(wl_bss_info_t *bi);
extern int  get_roam_channel_list(int target_chan,
	chanspec_t *channels, const wlc_ssid_t *ssid, int ioctl_ver);
extern void print_roam_cache(void);
extern void set_roam_band(int band);
	...
	enum wl_handler_del_type type)
{
	...
	if (cfg->pm_enable_work_on) {
		if (add_remove) {
		    /* david */
			bool ret = cancel_delayed_work_sync(&cfg->pm_enable_work);
			if (ret) WL_ERR(("PM-WORK: recovered from dup work\n"));

			schedule_delayed_work(&cfg->pm_enable_work,
				msecs_to_jiffies(WL_PM_ENABLE_TIMEOUT));
			//WL_ERR(("PM-WORK: started, dur=%d\n", WL_PM_ENABLE_TIMEOUT)); /* david */
		} else {
			...
			switch (type) {
				...
				case WL_HANDLER_PEND:
					...
					//WL_ERR(("PM-WORK: started, dur=%d\n", WL_PM_ENABLE_TIMEOUT*2)); /* david */
					break;
				case WL_HANDLER_DEL:
				default:
					cfg->pm_enable_work_on = false;
					//WL_ERR(("PM-WORK: pm_enable_work_on = false\n")); /* david */
					break;
			}
		}
	}
...
#define LOCAL_BUF_LEN 1024
	...
	/*add protect for channel spec get-op*/
	if (dtoh32(list->count) > htod32(WL_NUMCHANSPECS)){
		memset(pbuf, 0x0, LOCAL_BUF_LEN);
		list = (wl_uint32_list_t *)(void *)pbuf;
		list->count = htod32(WL_NUMCHANSPECS);
		...
		if (dtoh32(list->count) > htod32(WL_NUMCHANSPECS)){
			WL_ERR(("get chanspecs failed after retry\n"));
			kfree(pbuf);
			return -1;
		}
	}
...
#undef LOCAL_BUF_LEN
	...
	for (i = 0; (i < dtoh32(list->count))&&(i < htod32(WL_NUMCHANSPECS)); i++) {
		index = 0;
		update = false;
		ht40_allowed = false;

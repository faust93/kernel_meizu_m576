...
#define MAX_NVRAMBUF_SIZE	8192	/* max nvram buf size */
...
static struct pm_qos_request cl1_min_qos_req;
static struct pm_qos_request cl1_max_qos_req;
static struct pm_qos_request cl0_min_qos_req;
static struct pm_qos_request cl0_max_qos_req;
static struct pm_qos_request mif_qos_req;
static struct pm_qos_request int_qos_req;
static struct pm_qos_request cl1_min_num_req;
static struct pm_qos_request cl1_max_num_req;
...
static void qos_unlock(void)
{
	...
	REMOVE_PM_QOS(&cl1_max_qos_req);
	REMOVE_PM_QOS(&cl0_min_qos_req);
	REMOVE_PM_QOS(&cl0_max_qos_req);
	REMOVE_PM_QOS(&mif_qos_req);
	REMOVE_PM_QOS(&int_qos_req);
	REMOVE_PM_QOS(&cl1_min_num_req);
	REMOVE_PM_QOS(&cl1_max_num_req);
	REMOVE_PM_QOS(&cl0_min_num_req);
	REMOVE_PM_QOS(&cl0_max_num_req);
}

static unsigned int qos_wlan[][14]={
	{150*1024*1024/8, 2100000, 2100000, 1500000, 0, 1552000, 560000,4,4,4,4,1,1,1},
	{150*1024*1024/8, 1500000, 1500000, 1500000, 0, 1552000, 560000,4,4,4,4,1,1,1},
	{100*1024*1024/8, 0, 0, 1200000, 0, 1026000, 400000,0,0,4,4,0,0,0},
	{50*1024*1024/8, 0, 0, 1000000,  0, 0, 200000,0,0,4,4,0,0,0}
};


static void qos_lock(int level)
{
	unsigned cl1_min_freq, cl1_max_freq, cl0_min_freq, cl0_max_freq, mif_freq, int_freq,
		cl1_min_num, cl1_max_num, cl0_min_num, cl0_max_num, task_affinity_en,irq_affinity_en,hmp_boost_en;

	printk("******level:%d,%s\n",level,__FUNCTION__);
	cl1_min_freq = qos_wlan[level][1];
	cl1_max_freq = qos_wlan[level][2];
	cl0_min_freq = qos_wlan[level][3];
	cl0_max_freq = qos_wlan[level][4];
	mif_freq = qos_wlan[level][5];
	int_freq = qos_wlan[level][6];
	cl1_min_num = qos_wlan[level][7];
	cl1_max_num = qos_wlan[level][8];
	cl0_min_num = qos_wlan[level][9];
	cl0_max_num = qos_wlan[level][10];
	task_affinity_en = qos_wlan[level][11];
	irq_affinity_en = qos_wlan[level][12];
	hmp_boost_en = qos_wlan[level][13];

	if (cl1_min_freq) {
		UPDATE_PM_QOS(&cl1_min_qos_req,
			PM_QOS_CLUSTER1_FREQ_MIN, cl1_min_freq);
	} else {
		REMOVE_PM_QOS(&cl1_min_qos_req);
	}

	if (cl1_max_freq) {
		UPDATE_PM_QOS(&cl1_max_qos_req,
			PM_QOS_CLUSTER1_FREQ_MAX, cl1_max_freq);
	} else {
		REMOVE_PM_QOS(&cl1_max_qos_req);
	}

	if (cl0_min_freq) {
		UPDATE_PM_QOS(&cl0_min_qos_req,
			PM_QOS_CLUSTER0_FREQ_MIN, cl0_min_freq);
	} else {
		REMOVE_PM_QOS(&cl0_min_qos_req);
	}

	if (cl0_max_freq) {
		UPDATE_PM_QOS(&cl0_max_qos_req,
			PM_QOS_CLUSTER0_FREQ_MAX, cl0_max_freq);
	} else {
		REMOVE_PM_QOS(&cl0_max_qos_req);
	}

	if (mif_freq) {
		UPDATE_PM_QOS(&mif_qos_req,
	...
	if (cl1_max_num) {
		UPDATE_PM_QOS(&cl1_max_num_req,
			PM_QOS_CLUSTER1_NUM_MAX, cl1_max_num);
	} else {
		REMOVE_PM_QOS(&cl1_max_num_req);
	}
	if (cl0_min_num) {
		UPDATE_PM_QOS(&cl0_min_num_req,
			PM_QOS_CLUSTER0_NUM_MIN, cl0_min_num);
	} else {
		REMOVE_PM_QOS(&cl0_min_num_req);
	}

	if (cl0_max_num) {
		UPDATE_PM_QOS(&cl0_max_num_req,
			PM_QOS_CLUSTER0_NUM_MAX, cl0_max_num);
	} else {
		REMOVE_PM_QOS(&cl0_max_num_req);
	}
}
...
bool dhd_bus_watchdog(dhd_pub_t *dhd)
{
    ...
    if(dhd_second_count >= ( 1000 / CUSTOM_DHD_WATCHDOG_MS)) {
        unsigned long tx_rate = (dhd->dstats.tx_bytes - dhd_last_txbytes);
        unsigned long rx_rate = (dhd->dstats.rx_bytes - dhd_last_rxbytes);
        static bool status_bl1 = false;

		old_level = current_level;
       if  (tx_rate > qos_wlan[0][0]) {
            current_level = 0;
        } else if  (rx_rate > qos_wlan[1][0]) {
            current_level = 1;
        } else if  (tx_rate > qos_wlan[2][0] || rx_rate > qos_wlan[2][0]) {
            current_level = 2;
        } else if  (tx_rate > qos_wlan[3][0] || rx_rate > qos_wlan[3][0]) {
            current_level = 3;
        } else {
            current_level = 100;
        }
	...
	if(old_level != current_level){
		if(100 == current_level) 
			...
			qos_lock(current_level);
	}

        dhd_last_txbytes = dhd->dstats.tx_bytes;
        dhd_last_rxbytes = dhd->dstats.rx_bytes;
        dhd_second_count = 0;
    }
#endif
	return FALSE;
...
dhdpcie_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
	...
	if (image == NULL) {
		DHD_ERROR(("%s: failed to open file %s\n", __FUNCTION__, pfw_path));
		goto err;
	}
...
_dhdpcie_download_firmware(struct dhd_bus *bus)
{
	...
	DHD_ERROR(("%s(line %d): atfer download firmware and nvram take arm out of reset\n", __FUNCTION__, __LINE__));
...
dhdpcie_bus_membytes(dhd_bus_t *bus, bool write, ulong address, uint8 *data, uint size)
{
	...
	bool little_endian;
#define CONFIG_64BIT_ALIGN
#if defined(CONFIG_ARCH_MSM8994) || defined(CONFIG_64BIT_ALIGN)
...
dhdpcie_bus_write_vars(dhd_bus_t *bus)
{
	...
	DHD_ERROR(("Physical memory size: %d, usable memory size: %d\n",
		phys_size, bus->ramsize));
	DHD_ERROR(("Vars are at %d, orig varsize is %d bcmerror=%d\n",
		varaddr, varsize, bcmerror));
...
dhdpcie_readshared(dhd_bus_t *bus)
{
	...
	if ((addr == 0) || (addr == bus->nvram_csm) || (addr < bus->dongle_ram_base) ||
		...
		DHD_ERROR(("%s: address (0x%08x) of pciedev_shared invalid\n",
			__FUNCTION__, addr));
		printk("%s: nvram_csm (0x%08x), dongle_ram_base(0x%08x), shaddr (0x%08x)\n", __func__, bus->nvram_csm, bus->dongle_ram_base, shaddr);

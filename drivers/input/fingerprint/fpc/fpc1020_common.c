...
static const struct chip_struct chip_data[] = {
	...
	{FPC1020_CHIP_1155X, 0x9500, 1, FPC1150_ROWS, FPC1150_COLUMNS, FPC102X_ADC_GROUP_SIZE},
...
const fpc1020_setup_t fpc1020_setup_default_1150_a1b1f1 = {
	...
	.capture_finger_up_threshold	= 0,
	.capture_finger_down_threshold	= 1,
	.finger_detect_threshold	= 0x20,
...
int fpc1020_check_hw_id(fpc1020_data_t *fpc1020)
{
	...
	if (match) {
		...
		fpc1020->chip.spi_max_khz    = 4800; //chip_data[counter].spi_max_khz;
...
#ifdef AS_HOME_KEY
int fpc1020_wait_finger_present_timeout(fpc1020_data_t *fpc1020)
{
	int error = 0;
	...
	//while (1) {
		error = fpc1020_wait_for_irq(fpc1020,
						10);//wait only 100ms,to check double tab
					...
					int measured_val)
{
	...
	#else
	new_val = measured_val + 15;//1;//16; //450 480 raw pixelsum Todo: awaiting calculated values
    #endif
	...
	if (!error) {
		...
		//dev_dbg(&fpc1020->spi->dev, "%s %d -> %d\n",__func__,old_val,fpc1020->setup.finger_detect_threshold);
	} else {
		dev_err(&fpc1020->spi->dev,

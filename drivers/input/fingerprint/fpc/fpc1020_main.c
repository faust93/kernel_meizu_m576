...
static int  fpc1020_supply_init(fpc1020_data_t *fpc1020, struct device *dev);
...
static ssize_t fpc1020_nav_switch_write(struct file *file, const char __user *buffer, size_t count, loff_t *pos)
{
	...
	// prevent setting nav mode while screen is off
	if(nav_switch == 1 && !enable_keys)
	    return count;
	else
...
#ifndef VENDOR_EDIT //changhua.li for debug
    int cs;
...
//return 0;
	...
	error = fpc1020_supply_init(fpc1020, dev);
	if (error)
		goto err;
...
*/
	regulator_l22 = regulator_get(dev, "vdd28_fp");
	if (IS_ERR(regulator_l22)) {
		printk("[fp][FPC]Fail to get l22 regulator source\n");
		retval = PTR_ERR(regulator_l22);
		return retval;
	}
	
	retval = regulator_enable(regulator_l22);
	if (retval) {
		printk("[fp][FPC]Fail to enable l22 regulator\n");
		return retval;
	}

	retval = gpio_direction_output(fpc1020->reset_gpio, 1);
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->reset_gpio)) {
		...
		error = gpio_request(pdata->reset_gpio, "fpc,gpio_reset");
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->irq_gpio)) {
		...
		error = gpio_request(pdata->irq_gpio, "fpc,gpio_irq");
	...
	if (error) {
        ...
        irq_set_irq_wake(fpc1020->irq,1);
...
of_err:
	...
	pdata->fp2050_gpio= of_get_named_gpio(node, "fpc,use_fp2050", 0);
